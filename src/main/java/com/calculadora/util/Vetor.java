...
*/
    ...
    public double distanceTo(Vetor that) {
        if (this.length() != that.length()) throw new RuntimeException("Dimensions don't agree");
        return this.minus(that).magnitude();
    }

    // return this + that
    public Vetor plus(Vetor that) {
        if (this.length() != that.length()) throw new RuntimeException("Dimensions don't agree");
        Vetor c = new Vetor(N);
        for (int i = 0; i < N; i++)
    ...
    public Vetor times(double factor) {
        Vetor c = new Vetor(N);
        for (int i = 0; i < N; i++)
    ...
    public static void main(String[] args) {
        ...
        Vetor x = new Vetor(xdata);
        Vetor y = new Vetor(ydata);
